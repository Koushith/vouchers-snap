...
export class Wallet {
  ...
  ): Promise<ServiceResponse<WalletSecret>> {
    try {
  ...
  async send(to: string, value?: string): Promise<TransactionResponse> {
    try {
      const balance = await this.walletProvider.getBalance();
      const gasPrice = await this.walletProvider.getGasPrice();
      const gasFee = parseFloat(utils.formatEther(gasPrice)) * 45000;
      const transferValue = value ? parseFloat(value) : parseFloat(utils.formatEther(balance)) - gasFee;

      if (transferValue < parseFloat(utils.formatEther(balance))) {
        const tx = {
          to: to,
