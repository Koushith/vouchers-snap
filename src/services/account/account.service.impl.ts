...
import { SafientCore, Types, Errors } from '@safient/core';
...
export class AccountServiceImpl extends Service implements AccountService {
  ...
  async registerEphemeral(name: string, email: string, pKey?: string): Promise<ServiceResponse<Types.User>> {
    try {

      const wallet = new Wallet();
      const account = await wallet.load('kovan', {privateKey: pKey});
      const safient = new SafientCore(this.accountStore.network);
      const user = await safient.createUser(account.data!, {name: name, email: email}, false);

      return this.success<Types.User>(user.data!);
    } catch (e: any) {
      return this.error<Types.User>(e);
    }
  }

  async loginEphemeral(privateKey: string): Promise<ServiceResponse<Types.User>> {
    try {

      const safient = new SafientCore(this.accountStore.network);
      this.accountStore.setSafient(safient)
      const wallet = new Wallet();
      const account = await wallet.load('kovan', {privateKey: privateKey});
      const user = await safient.loginUser(account.data!);
